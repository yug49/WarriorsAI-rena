...
exports.commandsRouter = exports.gameStates = void 0;
const express_1 = require("express");
const router = (0, express_1.Router)();
// Command queue for frontend polling
const commandQueue = new Map();
exports.gameStates = new Map(); // Export for status endpoint
const activeTimers = new Map();
// Timer management - sends commands to frontend
function startCommandTimer(battleId, intervalMs = 1000) {
    stopCommandTimer(battleId);
    console.log(`⏰ Starting command timer for battle ${battleId}`);
    const timer = setInterval(async () => {
        const state = exports.gameStates.get(battleId);
        if (!state) {
        ...
        if (state.timeRemaining <= 0) {
            ...
            if (state.phase === 'startGame') {
                ...
                exports.gameStates.set(battleId, state);
...
function stopCommandTimer(battleId) {
    ...
    try {
        ...
        if (command) {
            ...
            return res.status(200).json({
                hasCommand: true,
                command: command,
                gameState: exports.gameStates.get(battleId)
            });
        }
        // Return current state - Return 200 even if no gameState to prevent 404 errors
        const gameState = exports.gameStates.get(battleId);
    ...
    try {
        ...
        switch (action) {
            case 'initialize':
                ...
                exports.gameStates.set(battleId, newGameState);
            ...
            case 'reset':
                ...
                const currentState = exports.gameStates.get(battleId);
                if (currentState) {
                    ...
                    exports.gameStates.set(battleId, currentState);
                    return res.status(200).json({
